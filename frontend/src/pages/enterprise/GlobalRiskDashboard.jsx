...
import RealTimeCommodityPrices from '../../components/enterprise/RealTimeCommodityPrices';
...
export default function GlobalRiskDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Try contract-specific first, then fall back to global (all user contracts)
      let apiData;
      try {
        apiData = await enterpriseRiskService.getDashboardData(contractId || '');
      } catch {
        apiData = await enterpriseRiskService.getDashboardData('');
      }

      const totalExposure = apiData.total_exposure || 0;
      const var95 = apiData.var_95 || totalExposure * 0.25;
      const var99 = apiData.var_99 || totalExposure * 0.35;

      setData({
        exposure: {
          base_exposure: totalExposure,
          total_exposure: totalExposure * 1.15,
          systemic_multiplier: 1.15
        },
        monte_carlo: { mean: totalExposure * 1.05, p95: var95, p99: var99 },
        commodity_risk: { commodity_risk_exposure: totalExposure * 0.1 },
        margin: apiData.expected_margin && totalExposure > 0
          ? (apiData.expected_margin / totalExposure) * 100
          : 18.5,
        supply_chain_risk_score: apiData.supply_chain_risk_score || 0,
        geo_political_risk_score: apiData.geo_political_risk_score || 0,
        trend: { exposure: 0, margin: 0, var: 0 },
        exposure_breakdown: apiData.exposure_breakdown || [],
        top_risky_contracts: apiData.top_risky_contracts || [],
        total_contracts: apiData.total_contracts || 0,
        high_risk_contracts: apiData.high_risk_contracts || 0,
      });
    } catch (err) {
  ...
  const quickLinks = [
    ...
    },
    {
      ...
      description: 'Coverage & liability transfer',
      icon: Shield,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">
                {contractId ? 'Contract-Specific Analysis' : 'Aggregate View — All Contracts'}
              </p>
            </div>
          </div>
          <ContractSelector
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quickLinks.map((link, index) => {
            ...
            return (
              <button
                key={index}
                onClick={() => navigate(contractId ? `${link.path}?contractId=${contractId}` : link.path)}
